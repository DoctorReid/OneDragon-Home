...
7. 游戏内「设置」-「其他」-「记录棋盘加速设置」选择「开启」，第一次进入空洞后手动选择一次加速。可以让后续刷空洞稍微快一点。
8. 游戏内「设置」-「输入」-「镜头自动跟随转动」选择「关闭」，可以让战斗场景内的按键不改变视角，更方便移动脱困。
9. 游戏内的副本开启沉浸模式，避免刷本时被凌晨过夜卡住。
10. 完成脚本设置，主要是改键部分，见[设置说明](config.md)
11. 使用过程中请多查看 [各功能说明](./docs/feat_one_dragon.md)
